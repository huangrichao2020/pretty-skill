...
MIN_UNIQUE_COLORS = 800          # 唯一像素色 < 800 几乎一定是色块 / SVG 描边图（v3.18 从 1000 降到 800 · 信息图风格真出图最低 ≈ 967）
...
# v3 预设 12 个中文领域（v3.18 起全面中文化 · 去掉英文别名 · 新增「玄学修炼」）
PRESET_DOMAINS = {
    ...
    "橙皮书": "5 步流水线 / 调研 / 规划 / 写作 / 三审三校 / 构建成书",
    "社交主导": "社交主导权 / 社会博弈 / 权力动态",
    "视觉创作": "知识生图 / AI 直生图 / 视觉内容生产",
    "故事写作": "剧情推演 / 真实故事转剧情 / 角色 / 潜台词",
}
...
def check_real_images(case_dir: Path) -> tuple[bool, list[str]]:
    ...
    检测规则（任一失败 → 报错）：
      ...
      3. 唯一像素色 ≥ 800（v3.18 修复 · 信息图风格真出图最低 ≈ 967 · PIL 凑合图几乎不可能 > 500）
      4. 纵横比在 16:9 / 4:3 / 3:4 / 9:16 / 1:1 之一

    v3.18 阈值修复说明：
      - v3.16 原阈值 1000 把橙皮书/宏观雷达等信息图类真出图（967-994）误判为凑合
      - 实际数据：手绘多色类 8000-13000 色 / 信息图类 950-1200 色 / PIL 凑合图 50-300 色
      - 800 是「凑合图 vs 真 AI 出图」的安全分界
