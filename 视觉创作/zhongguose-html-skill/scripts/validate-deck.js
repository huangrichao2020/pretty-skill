...
if (!target) {
  console.error('Usage: node scripts/validate-deck.js path/to/index.html');
  process.exit(2);
}
...
if (/<title>[^<]*中国色 HTML 模板[^<]*<\/title>/.test(html)) {
  warnings.push('Template default <title> (中国色 HTML 模板 · 示例) is still present. Replace it for a real deck.');
}
