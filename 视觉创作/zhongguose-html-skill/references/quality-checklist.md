...
- [ ] 修改 HTML 后已重新生成并内联 WOFF2，同时更新 manifest。
- [ ] The WOFF2 was rebuilt and embedded, and the manifest updated, after the last HTML edit.
...
- [ ] OFL-1.1.txt、FONT-NOTICE.txt 与 manifest 位于 `assets/fonts/`，WOFF2 已嵌入 HTML。
- [ ] OFL-1.1.txt, FONT-NOTICE.txt, and the manifest sit in `assets/fonts/`; the WOFF2 is embedded in the HTML.

## 5. 浏览器与交互 / Browser and interaction

执行：

    node "<skill_dir>/scripts/browser-qa.js" "path/to/index.html"
...
本地预览：
    ...
    node "<skill_dir>/scripts/preview-deck.js" "path/to/index.html"
...
- [ ] 已替换所有占位文案（含 `占位` 字样与示例框架标签 起/承/转/合）。
- [ ] Replaced all placeholder copy (text containing `占位` and the sample framework labels 起/承/转/合).
