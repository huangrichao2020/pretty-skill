---
name: zhongguose-html-skill
description: 以原稿章法为骨架，中国传统色彩为气韵，**直接产出可交互、可迁移的中式 HTML 汇报演示稿**。用于需要梳理叙事逻辑、章节框架、页级结构或版式的演示/汇报，且内容涉及中国文化，或视觉希望采用中国色、中国风、中式/东方审美；点名本 Skill、模板或框架导航格式时也触发。**只支持 HTML 交付**：输出 `index.html` + `assets/` 目录（自包含、字体嵌入、可双击打开）。不生成 PPT/PPTX。 Build a Chinese-style HTML presentation system that maps confirmed source material into faithful reporting logic, sections, slide architecture, and layouts, then renders a self-contained interactive HTML deck. Use when presentation work involves Chinese culture or requests Chinese-color, Chinese-style, or Eastern aesthetics, and when this Skill, template, or framework navigation is named. HTML-only delivery; no PPT/PPTX mode.
---

# Zhongguose HTML Presentation Skill / 中国色 HTML 汇报演示 Skill

## Credits / 署名

- 本 skill 由【木渡川】制作。/ This skill is created by 木渡川 (Muduchuan).
- 本 skill 由 **zhongguose-ppt-skill 改造而来**：去掉了原 PPT 模式分支，纯化为 HTML-only skill。/ This skill is refactored from `zhongguose-ppt-skill` (ppt-only branch removed, HTML-only mode).
...
Build a **self-contained interactive HTML deck** (one `index.html` plus an `assets/` directory) that maps confirmed source material into faithful reporting logic, sections, slide architecture, and layouts. The deck is Chinese-led bilingual, uses one Chinese-traditional theme color, Chinese cultural atmosphere, restrained grid discipline, and a logic-first narrative arc.

**只支持 HTML 交付**：本 skill 不生成或编辑 `.pptx` / `.key` / Google Slides 等其他格式。如需其他格式，请使用专门的 PPT 制作 skill，本 skill 仅作为其视觉与逻辑合同的输入。

构建一套**自包含的交互式 HTML 汇报演示**（一个 `index.html` 加 `assets/` 文件夹），把已确认的源文档映射为忠实的汇报逻辑、章节、页级架构与版式。中英双语、单一中国传统色、克制网格、逻辑优先。

### Routing Boundary / 触发边界

Use this routing formula: presentation or reporting structure work AND (Chinese-cultural content OR Chinese-style visual intent) AND (HTML deck output expected). Naming this Skill, its template, or its framework-navigation format is an explicit override.

使用以下路由公式：演示或汇报结构任务 AND（中华文化内容 OR 中式视觉诉求）AND（要 HTML 输出）。用户点名本 Skill、对应模板或框架导航格式时，视为明确调用。
...
仅使用中文或仅涉及中国市场，并不等于具备中华文化或中式风格条件。没有相关文化/审美诉求的通用 deck 转换，以及 logo、海报等非演示类任务，均不触发。
...
The following runtime dependencies are required: Node.js 16+ for the `.js` outline, validation, preview, and browser-QA scripts; Python 3.10+ with FontTools and Brotli to rebuild the deck-specific cover font. The bundled `package.json` keeps the scripts in ESM mode.

需要以下运行依赖：Node.js 16+（跑 `.js` 大纲抽取、校验、预览与浏览器 QA 脚本）；Python 3.10+ + FontTools + Brotli（重建逐稿封面字体）。内置 `package.json` 会保持 ESM 模式。

- Node.js 16+ is required for the `.js` outline, validation, preview, and browser-QA scripts. The bundled `package.json` keeps them in ESM mode.
- 运行 `.js` 大纲抽取、校验、预览与浏览器 QA 脚本需要 Node.js 16+；内置 `package.json` 会保持 ESM 模式。
...
This skill only delivers HTML. Follow the complete workflow below and ship a self-contained interactive deck.

本 skill 只交付 HTML。执行下方完整工作流，交付自包含的交互演示。

0. Confirm theme color, manuscript status, and polishing scope.
   先确认主题色、文稿状态与润色尺度。
   Before visual-design work begins, ask which theme color to use, listing the options from the theme-color table in `references/visual-system.md` (name + hex); default to 飞泉绿 if the user has no preference. When asking, open `assets/theme-cover-gallery.html` (under this skill's folder) in the browser so the user can compare cover samples before choosing. Set `--accent` and `--accent-on` as a pair per that table. Theme selection may run in parallel with source mapping and slide architecture — do not block one on the other.
   进入视觉设计阶段前，要询问用户选哪种主题色，并把 `references/visual-system.md` 主题色表的可选项（名称 + 色值）列出；询问时同时在浏览器打开本 skill 目录下的 `assets/theme-cover-gallery.html`，让用户对照封面示例再选。用户无偏好时默认飞泉绿，并按表成对设置 `--accent` 与 `--accent-on`。主题色选择可与源文档梳理、页级架构并行，不互相阻塞。
...
4. Start from `assets/template-zhongguose/`.
   ...
   Copy the whole folder to the target HTML output directory and edit `index.html`. Keep `assets/motion.min.js`, `assets/lucide.min.js`, `assets/licenses/`, and `assets/fonts/` beside the HTML. The seed cover-font subset and placeholder artwork are embedded as data URLs in the HTML; the full source font is downloaded to a user cache only when the subset is rebuilt.
   将整个模板文件夹复制到目标输出目录并编辑 `index.html`。保留同级 `assets/motion.min.js`、`assets/lucide.min.js`、`assets/licenses/` 与 `assets/fonts/`。种子封面字体子集和占位图已以内联 data URL 写入 HTML；只有重建子集时才会把完整源字体下载到用户缓存。

   The template is seeded with placeholder demo content. For a real deck, always replace:
   模板内置的是占位示例内容。制作正式 deck 时，必须替换：

   - The `<title>` (`中国色 HTML 模板 · 示例`).
   - `<title>`（`中国色 HTML 模板 · 示例`）。
   ...
   - All embedded placeholder images (the `data:image/svg+xml;base64,...` image sources with placeholder alt text).
   - 所有内联占位图（`src` 以 `data:image/svg+xml;base64,...` 开头、`alt` 标注为占位图的图像）。
...
8. Rebuild the deck-specific cover font after the HTML is final.
   HTML 定稿后重建逐稿封面字体。
   Run the bundled script after all slide copy and layout edits (`<skill_dir>` is the folder containing this SKILL.md). On first use it downloads the pinned qiji-font 0.0.4 source to the user cache and verifies its SHA-256. It then extracts visible characters, builds a temporary WOFF2 subset, embeds that subset into the HTML `@font-face`, copies the OFL license and font notice, and records the HTML/font hashes in `zhongguose-cover.manifest.json`.
   所有页面文案与排版编辑完成后运行内置脚本（`<skill_dir>` 指本 SKILL.md 所在文件夹）。首次运行会把固定版本的 qiji-font 0.0.4 源字体下载到用户缓存并校验 SHA-256；随后提取可见字符、生成临时 WOFF2 子集，将子集嵌入 HTML 的 `@font-face`，复制 OFL 许可证和字体说明，并在 `zhongguose-cover.manifest.json` 中记录 HTML/font 哈希。
   ...
   When rebuilding the bundled seed template, include the theme-cover gallery so all six cover names remain in the embedded WOFF2 subset:
   重建本 Skill 自带的种子模板时，必须同时纳入主题色封面示例，确保六组封面名称都保留在内联 WOFF2 子集中：
   ...
   Run it again after every subsequent HTML change. The first run needs network access unless the verified source is already cached; the final deck never contains the full source font.
   此后只要 HTML 再发生变化，就重新运行。首次运行需要联网，除非已存在通过校验的缓存源字体；最终 deck 不会包含完整源字体。
...
9. Extract and compare the deck outline.
   ...
   ```bash
   node <skill_dir>/scripts/extract-deck-outline.js path/to/index.html
   ```
...
10. Validate the deck.
   ...
   ```bash
   node <skill_dir>/scripts/validate-deck.js path/to/index.html
   ```
...
The following CSS variables and class rules are the source of truth for the HTML deck. When handing the deck to another rendering tool, translate them into equivalent theme-color tokens, typography hierarchy, and reusable page-component rules.

下列 CSS 变量与 class 规则是 HTML deck 的权威定义。如果要把同一份合同交给其他渲染工具，按这里的规则转换为等价的主题色 token、字体层级与可复用页面组件规范。
...
Network dependency note: motion and lucide load from local `assets/` first and only fall back to a CDN, so animations and icons work offline. The deck-specific `Zhongguose Cover` WOFF2 subset is embedded in `index.html` and works offline after generation. Rebuilding that subset needs one-time access to the pinned source URL unless the verified source is cached. Other web fonts (Orbitron, Noto Serif SC, LXGW WenKai, Inter, JetBrains Mono) still load from `fonts.googleapis.com`; without access to it the deck falls back to system fonts and the typography loses most of its character. Tell the user about this limitation if the deck must work fully offline or in a network that blocks Google Fonts.

联网依赖说明：motion 和 lucide 均优先加载本地 `assets/` 文件，仅在缺失时回退 CDN，离线也能保证动效和图标。逐稿生成的 `Zhongguose Cover` WOFF2 子集会嵌入 `index.html`，生成后可离线使用；重建子集时需要首次访问固定源地址，已有通过校验的缓存则不需要。其余 Web 字体（Orbitron、Noto Serif SC、LXGW WenKai、Inter、JetBrains Mono）仍从 `fonts.googleapis.com` 加载；无法访问时会回退系统字体，版式不变但字体气质会明显打折。如果 deck 需要完全离线或在无法访问 Google Fonts 的网络中放映，需向用户说明这一限制。
