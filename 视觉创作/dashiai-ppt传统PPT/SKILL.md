...
借鉴 CyberPPT 三段式:**证据链 → 视觉蓝图 → PPTX 重建**。dashiai-ppt 的完整 12 步:

### 段1: 证据链 (先锁定"凭什么成立")

1. 提炼用户目标: `title`、`goal`、`audience`、`owner`、页数、内容重点和最终产物格式。用户未指定页数时默认 10 页左右,不少于 8 页。
2. **先写证据链**:对每一页,在 `slides[].evidence` 里填:
   - `conclusion`: 这页要证明什么(一句话)
   - `evidence_ids[]`: 引用 text 字段中的数据 key
   - `conflicts[]`: 需要回避或说明的矛盾数据(可选)
   **先过证据关,再进视觉关**——如果某页说不出"凭什么成立",先补证据再选 layout。
3. 确认 `themePack`。用户未指定时先询问风格;用户选定后生成 `randomSeed`,例如 `<主题>-<日期>-<3位随机词>`,保证随机选页可复现。
4. 判断图片意图:无图但需要视觉素材时先问是否预留图片槽;用户给本地素材先 `media:stage`;明确生图时用 image-gen。

### 段2: 视觉蓝图 (结构化填充)

5. 用 `layout:query` 选候选;对象/数组/count/图片 props 用 `inspect:layout` + `props:safe`。
  ...
  "slides": [
    {
      ...
      "evidence": {
        "conclusion": "美国 AI 融资规模在 2024 年创历史新高",
        "evidence_ids": ["totalFunding", "investorCount"]
      },
      "props": {"kicker": "融资调研 · VOL.01", "titleTop": "美国 AI", "titleBottom": "融资调研", "lead": "从资本体量、赛道结构和典型公司拆解本轮 AI 融资周期。"}}
    ,
    {
      "layout": "theme01_page006",
      "evidence": {
        "conclusion": "基础模型是本轮吸金最多的赛道",
        "evidence_ids": ["topSector"]
      },
      "props": {"kicker": "核心数字", "value": "970", "unit": "亿美元", "sub": "2024 年美国 AI 风险投资规模创历史新高。"}}
    ,
    {
      "layout": "theme01_page010",
      "evidence": {
        "conclusion": "横纵交叉分析能有效识别融资结构变化",
        "evidence_ids": []
      },
      "props": {"kicker": "# 研究方法", "title": "横纵分析法", "cn": "用时间维度和赛道维度交叉识别融资变化。"}}
    ,
    {
      "layout": "theme01_page030",
      "evidence": {
        "conclusion": "头部公司融资节奏快于中腰部但估值分歧增大",
        "conflicts": ["估值分歧数据尚未完全公开"]
      },
      "props": {"kicker": "# 典型案例", "title": "里程碑 · 头部公司融资节奏"}}
    ,
    {
      "layout": "theme01_page084",
      "props": {"kicker": "# 附录", "title": "数据来源与研究说明"}}
  ]
}
```

**注意**: `evidence` 不是必填字段,但建议为每一页关键结论都写上,这样即使不写 `evidence_ids` 也能帮助 AI 思考"这页凭什么成立"后再选 layout。
