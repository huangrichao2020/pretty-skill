...
def write_web_html(case_dir: Path, title: str, pages: list[dict]):
    ...
    placeholder = (
        f"# {pdf_name} · 生成占位\n\n"
        f"PDF 由 build_case_pdf.py 自动化生成。\n\n"
        f"用法：\n"
        f"```bash\n"
...
""")
    ...
    if not args.no_open:
        pdf_path = output_dir / f"{title}讲解.pdf"
        if pdf_path.exists():
            try:
                import platform, subprocess as _sp
                _sys = platform.system()
                if _sys == "Darwin":
                    _sp.Popen(["open", str(pdf_path.absolute())])
                    print(f"  🌐 已自动打开 PDF（macOS）: {pdf_path}")
                elif _sys == "Linux":
                    _sp.Popen(["xdg-open", str(pdf_path.absolute())])
                    print(f"  🌐 已自动打开 PDF（Linux）: {pdf_path}")
                elif _sys == "Windows":
                    _sp.Popen(["cmd", "/c", "start", "", str(pdf_path.absolute())])
                    print(f"  🌐 已自动打开 PDF（Windows）: {pdf_path}")
            except FileNotFoundError:
                print(f"  ⚠️  找不到 open/xdg-open/start 命令，请手动打开: {pdf_path}")
